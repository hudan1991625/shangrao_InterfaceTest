...
mysql_info = {"host": '172.24.0.11',
              "port": 3306,
              "user": 'm_jrft_sr_dbank',
              "passwd": 'CUn6J4',
              "db": 'jrft_sr_dbank',
              "charset": 'utf8'}
class MysqlUtil():
    ...
    def __getConnect(db_info):
     '''静态方法，从连接池中取出连接'''
     try:
         conn = MySQLdb.connect(host=db_info['host'],port=db_info['port'],user=db_info['user'],passwd=db_info['passwd'],db=db_info['db'],charset=db_info['charset'])
         return conn
...
if __name__ == "__main__":
    mysql = MysqlUtil()
    sql = "select * from `dbank_cal_schedule_info` where mobile='14140000001' and set_status='0' "
